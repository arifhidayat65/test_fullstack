...
class UserSignUp extends Component {
    ...
    checkErrors = ()=>{
      ...
      //check error pada saat konfirmasi password
      if(this.state.password !== this.state.confirmPassword){
        errors.push("Kata sandi tidak sama dengan Kata Sandi yang Dikonfirmasi!");
      }
      return errors;
    }
    ...
    handleSubmit = (event) => {
      ...
      //validasi check error
      const errors = this.checkErrors();
      if(errors.length>0){
        this.setState({errors});
        return;
      }
      //memanggil call pada login
      axios({
